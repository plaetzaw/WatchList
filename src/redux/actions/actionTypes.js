export const TESTCASE1 = "TESTCASE1";
export const TESTCASE_ADD = "TESTCASE-ADD";
export const TESTCASE_DELETE = "TESTCASE-DELETE";
export const LOADING_DATA = "LOADING_DATA";
export const SET_HOT_MOVIES = "SET_HOT_MOVIES";
export const SET_POPULAR_MOVIES = "SET_POPULAR_MOVIES";
export const SET_UPCOMING_MOVIES = "SET_UPCOMING_MOVIES";
export const ADD_TO_WATCHLIST = "ADD_TO_WATCHLIST";
export const REMOVE_FROM_WATCHLIST = "REMOVE_FROM_WATCHLIST";
export const GO_TO_MOVIE = "GO_TO_MOVIE";
export const FILTER_BY_GENRE = "FILTER_BY_GENRE";
export const SET_SINGLE_MOVIE = "SET_SINGLE_MOVIE";
export const BUILD_MOVIE_CREDITS = "BUILD_MOVIE_CREDITS";
export const BUILD_SIMILAR_MOVIES = "BUILD_SIMILAR_MOIVES";
export const BUILD_MOVIE_DETAILS = "BUILD_MOVIE_DETAILS";
